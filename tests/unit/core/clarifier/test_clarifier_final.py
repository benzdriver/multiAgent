...
class TestClarifier(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_run_llm(self):
        ...
        mock_result = {"result": "测试结果"}
        
        with patch('core.clarifier.clarifier.run_prompt', new=AsyncMock(return_value=mock_result)) as mock_run_prompt:
            result = await self.clarifier.run_llm("测试提示")
    ...
    async def test_deep_clarification(self, mock_file):
        ...
        with patch.object(self.clarifier, 'run_llm', new=AsyncMock(return_value=mock_result)):
    ...
    async def test_integrate_legacy_modules(self):
        ...
        with patch('importlib.import_module', side_effect=lambda x: mock_module) as mock_import:
