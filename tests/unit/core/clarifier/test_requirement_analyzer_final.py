...
class TestRequirementAnalyzer(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('json.loads')
    async def test_analyze_requirements(self, mock_json_loads):
        """测试分析需求文档"""
        mock_json_data = {
            "requirements": [
                {
        ...
        mock_json_loads.assert_called_once_with(mock_response["result"])

    @patch('json.loads')
    async def test_analyze_requirements_json_error(self, mock_json_loads):
        """测试分析需求文档 - JSON解析错误"""
    ...
    @patch('json.loads')
    async def test_analyze_granular_modules(self, mock_json_loads):
        """测试分析细粒度模块"""
        mock_modules = [
            {
        ...
        with patch.object(RequirementAnalyzer, '_save_granular_modules') as mock_save:
            ...
            mock_save.assert_called_once_with(mock_modules)

    @patch('json.loads')
    async def test_analyze_granular_modules_json_error(self, mock_json_loads):
        """测试分析细粒度模块 - JSON解析错误"""
        mock_json_loads.side_effect = json.JSONDecodeError("Invalid JSON", "", 0)
