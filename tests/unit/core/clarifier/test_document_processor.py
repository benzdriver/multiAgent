...
class TestDocumentProcessor(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_init_creates_dir(self, mock_mkdir):
        """测试初始化时创建目录"""
    ...
    @patch('pathlib.Path.mkdir')
    async def test_read_all_markdown_files_empty_dir(self, mock_mkdir):
        """测试读取空目录"""
        with patch('pathlib.Path.exists', return_value=False):
            processor = DocumentProcessor(input_path=Path("/nonexistent"))
    ...
    async def test_read_all_markdown_files_error(self, mock_read_text):
        """测试读取文件出错"""
        mock_read_text.side_effect = Exception("读取文件出错")
