...
class TestRequirementAnalyzerIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('core.clarifier.clarifier.Clarifier.clarify')
    async def test_integration_with_clarifier(self, mock_clarify):
        """测试与Clarifier的集成"""
        clarifier = Clarifier(
        ...
        mock_clarify.return_value = asyncio.Future()
        mock_clarify.return_value.set_result(None)
    ...
    async def test_integration_with_file_system(self, mock_write_text, mock_json_loads):
        """测试与文件系统的集成"""
        mock_json_data = {
            "system_overview": {
                "core_purpose": "测试系统",
    ...
    @patch('json.dump')
    async def test_integration_with_granular_modules(self, mock_json_dump, mock_file, mock_mkdir, mock_json_loads):
        """测试细粒度模块生成与文件系统集成"""
        mock_modules = [
            {
        ...
        mock_json_loads.return_value = mock_modules
        ...
        self.assertEqual(result, mock_modules)
