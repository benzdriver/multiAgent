...
class TestRequirementAnalyzerIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_integration_with_file_system(self, mock_write_text):
        """测试与文件系统的集成"""
    ...
    @patch('json.dump')
    async def test_integration_with_granular_modules(self, mock_json_dump, mock_file, mock_mkdir):
        """测试细粒度模块生成与文件系统集成"""
        mock_modules = [
            {
